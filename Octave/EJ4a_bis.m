...
yk = Pos(:,1:dim)+repmat(b0_p,1,cant_mediciones)' + M_eta';
...
C_b = [C [eye(dim)]]; % Concateno columnas de 0's excepto identidad en donde corresponda
R = diag([ones(1,dim*bool_p)*sigma_etap^2 ones(1,dim*bool_v)*sigma_etav^2 ones(1,dim*bool_a)*sigma_etaa^2]);
...
if bool_print
    print('../Informe/Figuras/graf_ej4a','-dpdf','-bestfit');
end
...
h2.PaperSize=wsize;
if bool_print
    print('../Informe/Figuras/bias_ej4a','-dpdf','-bestfit');
end
...
h3.PaperSize=wsize;
if bool_print
    print('../Informe/Figuras/covinn_ej4a','-dpdf','-bestfit');
end
