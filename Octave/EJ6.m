...
for k=1:length(factor_R)
    R = factor_R(k)*diag([ones(1,dim*bool_p)*sigma_etap^2 ones(1,dim*bool_v)*sigma_etav^2 ones(1,dim*bool_a)*sigma_etaa^2]);
...
for k=1:length(factor_R)
    ...
    suptitle(sprintf('R=%.2f',factor_R(k)));
    
    if bool_print
        print(sprintf(['../Informe/Figuras/graf_ej6_R' num2str(k)]),'-dpdf','-bestfit');
    end
    ...
    if bool_print
        print(sprintf(['../Informe/Figuras/covinn_ej6_R' num2str(k)]),'-dpdf','-bestfit');
    end
